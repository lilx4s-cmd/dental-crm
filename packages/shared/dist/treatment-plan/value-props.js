...
exports.VALUE_PROPS = void 0;
exports.valuePropsFor = valuePropsFor;
exports.brandLine = brandLine;
exports.VALUE_PROPS = [
    {
        condition: 'IMPLANT',
        title: 'Dental implants — a replacement that stands on its own',
        pitch: 'An implant is a titanium root placed into the jawbone, carrying its own crown. Unlike a bridge it does not lean on the teeth either side, so healthy teeth stay untouched, and unlike a denture it is fixed — you eat, speak and laugh without thinking about it. The bone grows onto the surface of the titanium and holds it the way it held the original root.',
        points: [
    ...
    },
    {
        ...
        pitch: 'A veneer is a thin facing bonded to the front of the tooth, changing its colour, shape and alignment while leaving the rest of the tooth alone. It is the conservative answer to a smile that looks worn, stained or uneven: far less of the tooth is removed than a crown would require.',
        points: [
    ...
    },
    {
        ...
        pitch: 'A bridge spans a missing tooth by anchoring to the teeth on either side, restoring the bite without surgery. Where the neighbouring teeth already need crowning, it is the efficient answer: one piece of work solves three problems at once.',
        points: [
    ...
    },
    {
        ...
        pitch: 'When infection reaches the nerve, the choice is to clean the canal out or to lose the tooth. Root canal treatment removes the infection and seals the space, so the tooth stays in your mouth and keeps doing its job. Kept under a crown afterwards, a treated tooth serves for many years.',
        points: [
    ...
    },
    {
        ...
        pitch: 'Where a tooth has been missing for a while, the bone that used to hold it thins out. A graft rebuilds that ridge so an implant has something solid to sit in. It is the step that decides whether the implant is stable in ten years, which is why we would rather add it than place an implant into bone that cannot carry it.',
        points: [
    ...
    },
    {
        ...
        pitch: 'In the upper back jaw the sinus often sits too low for an implant to be placed safely. A sinus lift raises that floor and packs bone beneath it, creating the height an implant needs. It is routine, planned from your CT scan, and it is what makes upper molar implants possible at all in many patients.',
        points: [
    ...
    },
    {
        ...
        pitch: 'A tooth that cannot be saved is removed as gently as the case allows, preserving the surrounding bone for whatever replaces it. The extraction is never the end of the plan: the replacement is decided before the tooth comes out, so you are not left with a gap and an open question.',
        points: [
    ...
    },
    {
        ...
        pitch: 'Decay is cleaned out and the tooth rebuilt in a composite matched to its own colour, bonded directly to the remaining tooth. Modern composite bonds to enamel rather than being wedged into a hole, so less healthy tooth has to be cut away to hold it.',
        points: [
    ...
    },
    {
        ...
        pitch: 'Hardened deposits below the gum line cannot be brushed away, and they are what loosens teeth over time. A professional clean removes them and gives everything else in this plan a healthy mouth to sit in. It is the least expensive item here and the one that protects the value of all the others.',
        points: [
...
function valuePropsFor(conditions) {
    const wanted = new Set();
    for (const c of conditions) {
        // A treated canal and a planned one sell the same way, as do the filling variants.
        if (c === 'ROOT_CANAL_TREATED')
            wanted.add('ROOT_CANAL');
        else if (c === 'AMALGAM_FILLING' || c === 'COMPOSITE_FILLING')
            wanted.add('FILLING');
        else
            wanted.add(c);
    }
    return exports.VALUE_PROPS.filter((v) => wanted.has(v.condition));
}
/**
 ...
 */
function brandLine(material, brand) {
    const m = material?.trim();
    const b = brand?.trim();
    if (b && m)
        return `Supplied for your case in ${m} by ${b}.`;
    if (b)
        return `Supplied for your case by ${b}.`;
    if (m)
        return `Supplied for your case in ${m}.`;
    return undefined;
}
