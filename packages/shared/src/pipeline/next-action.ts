...
// When a deal needs chasing, and what the chase is for.
//
// Deliberately rule-based rather than model-driven. "This deal has sat in Offer Sent for six days"
// is a fact the pipeline already knows, and deriving it is both free and correct — asking a
// language model to guess it would be slower, cost money per deal, and be wrong sometimes. The
// model earns its place further along, writing the actual message once the rules have decided who
// needs one.
//
// The cadences below are judgement, not physics. They are gathered here so the clinic can argue
// with a number in one place rather than hunt for it across the codebase.
...
export const TERMINAL_STAGES: PipelineStage[] = ['DONE', 'LOST'];
...
const wholeDaysBetween = (a: Date, b: Date) => Math.floor((a.getTime() - b.getTime()) / DAY_MS);
...
export function nextAction(
  ...
  if (TERMINAL_STAGES.includes(stage as PipelineStage)) {
  ...
  const urgency: ActionUrgency = now >= dueAt ? (overdueDays > 0 ? 'overdue' : 'due') : 'later';
...
export function dormantBefore(stage: string, now: Date = new Date()): Date | null {
  const cadence = STAGE_CADENCE[stage];
...
export function longestDormancyDays(): number {
  return Math.max(...Object.values(STAGE_CADENCE).map((c) => c.dormantAfterDays));
}
 ...
 */
export const RECYCLE_ANGLE: Record<string, string> = {
