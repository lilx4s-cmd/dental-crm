...
const BY_ID = new Map(TAG_COLORS.map((c) => [c.id, c]));

/** Falls back to slate: an unknown colour should render a plain pill, never an unstyled one. */
export function tagColorDef(color: string | null | undefined): TagColorDef {
  return BY_ID.get(color as TagColor) ?? TAG_COLORS[0];
}
...
export function normaliseTagName(raw: string): string {
  return raw.trim().replace(/\s+/g, ' ').slice(0, TAG_NAME_MAX);
}
 ...
 */
export const MAX_TAGS_PER_RECORD = 12;
